...
from .kernels import Kernel, get_D, get_psi, get_zeta, get_xi
...
#                 zeta = Zeta(L_f, h, -phi_n, phi_p)                              # TODO before was both phi_n
...
class BiasTosatto(BiasEstimator):
        ...
        def __call__(self, X):
                ...
                den = np.prod(get_zeta(self.k, -L_f, h, -delta_n, delta_p), axis=0) #np.prod(get_psi(self.k, L_f, h, -delta_n, delta_p), axis=0)
                # TODO:
                zeta = get_zeta(self.k, L_f, h, phi_n, phi_p) # phi_p??
                zeta = np.repeat(np.prod(zeta, axis=0, keepdims=True), d, axis=0)/zeta
                print("max zeta", np.max(zeta))
                A_k = get_xi(self.k, L_f, h, phi_n, phi_p)
                # assert np.all(A_k >= 0.), (-phi_n, phi_p)
                A = np.sum(L_m * A_k * zeta, axis=0)


                B = np.prod(get_zeta(self.k, L_f, h, -gamma_n, gamma_p), axis=0) - np.prod(get_zeta(self.k, L_f, h, phi_n, phi_p), axis=0)

                C = get_D(self.k, d, L_f, h, -gamma_n, gamma_p)
                ...
                #      C[np.ravel(X) >= u_p[0]] = 0.
                print("Maximum", np.max(B), np.max(C))
                print("max ret", np.max(ret), "min ret", np.min(ret))
                return ret
...
class BiasRosenblatt(BiasEstimator):
        ...
        def __call__(self, X):
                ...
                return (0.5 * self.m.second_derivate(X)
                       ...
                       * self.k.get_bandwidth()[0]**2 * self.k.get_rosenblatt_constant()